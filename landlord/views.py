...
#         if serializer.is_valid():
#             # Auto-set landlord — frontend never sends this field
#             serializer.save(landlord=landlord)
#             return Response(
...
#     if request.method == "GET":
#         # ✅ Safe query — only meetings linked to your properties
...
#         if serializer.is_valid():
#             serializer.save()
#             return Response({"message": "Meeting scheduled", "data": serializer.data}, status=201)
...
#     if request.method == "PUT":
#         s = MeetingSerializer(meeting, data=request.data, partial=True)
#         if s.is_valid(): s.save(); return Response({"message":"Updated", "data":s.data})
#         return Response({"error":s.errors},400)
#     if request.method == "DELETE":
#         meeting.delete(); return Response({"message":"Deleted"})
...
#         if serializer.is_valid():
#             serializer.save()
#             return Response({"message": "Payment recorded", "data": serializer.data}, status=201)
#         return Response({"error": serializer.errors}, status=400)
...
#         return Response({"error":s.errors},400)
#     if request.method == "DELETE":
#         pay.delete(); return Response({"message":"Deleted"})
...
#         if serializer.is_valid():
#             updated_lease = serializer.save()
#             # Sync property status automatically
#             if updated_lease.status == "ACTIVE":
#                 updated_lease.property.status = "OCCUPIED"
#             else:
#                 updated_lease.property.status = "AVAILABLE"
...
#         # Mark property as available before deleting
#         lease.property.status = "AVAILABLE"
#         lease.property.save(update_fields=['status'])
#         lease.delete()
#         return Response({"message": "Lease deleted successfully."})
